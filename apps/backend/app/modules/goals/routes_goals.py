...
def get_user_goals(request: Request):
    ...
    if not subscription["is_premium"] or subscription["plan_name"] not in ["Pro", "Gói Pro"]:
        raise HTTPException(status_code=403, detail="Career Goal Setting is a Pro feature. Please upgrade to Pro plan.")
    ...
    query = text(
        ...
        SELECT 
            ...
            (
                SELECT COUNT(*) 
                FROM core.goal_milestones m 
                WHERE m.goal_id = g.id AND m.status = 'completed'
...
def _get_roadmap_data(session: Session, career_id: str) -> dict:
    ...
    if not roadmap_row:
        return {"career_title": career_title, "milestones": []}
    ...
    return {"career_title": career_title, "milestones": milestones}
...
def generate_ai_milestones(request: Request, goal_id: int, payload: GenerateMilestonesRequest):
    ...
    subscription = SubscriptionService.get_user_subscription(user_id, session)
    if not subscription["is_premium"] or subscription["plan_name"] not in ["Pro", "Gói Pro"]:
        raise HTTPException(status_code=403, detail="AI Milestone Generation is a Pro feature.")
...
ONLY JSON, no other text."""
    ...
    try:
        ...
        for idx, m in enumerate(milestones_data):
            ...
            result = session.execute(
                insert_query,
                {"goal_id": goal_id, "title": title, "description": description, "target_date": target_date, "order_index": idx},
            )
...
def _create_fallback_milestones(
    session: Session, goal_id: int, career_name: str, target_months: int, recommended_months: int, time_warning: str
):
    ...
    if any(
        ...
        basic_milestones = [
            {
                "title": "Learn core programming language",
                "description": f"Master the main programming language for {career_name}. Complete basic exercises.",
                "weeks": 4,
            },
            {
                "title": "Build foundational knowledge",
                "description": "Learn data structures, algorithms, and core concepts.",
                "weeks": 6,
            },
            {
                "title": "Practice with small projects",
                "description": "Build 2-3 small projects to apply what you've learned.",
                "weeks": 6,
            },
            {
                "title": "Learn advanced frameworks/tools",
                "description": "Master popular frameworks and tools in the industry.",
                "weeks": 8,
            },
            {
                "title": "Build portfolio & job search",
                "description": "Complete portfolio with real projects, prepare CV and interviews.",
                "weeks": 4,
            },
        ]
    # Design careers
        ...
        basic_milestones = [
            {"title": "Learn design tools", "description": "Master Figma, Adobe XD or other main design tools.", "weeks": 4},
            {
                ...
                "description": "Learn typography, color theory, layout and composition.",
                "weeks": 4,
            },
        ...
        basic_milestones = [
            {
                ...
                "description": "Master basic marketing concepts and digital marketing.",
                "weeks": 4,
            },
        ...
        basic_milestones = [
            {
                ...
                "description": f"Deep dive into {career_name}, required skills and industry trends.",
                "weeks": 2,
            },
            {
                "title": "Learn foundational knowledge",
                "description": "Complete basic courses and get certifications if needed.",
                "weeks": 6,
            },
            {
                "title": "Develop practical skills",
                "description": "Practice through projects and real-world exercises.",
                "weeks": 8,
            },
