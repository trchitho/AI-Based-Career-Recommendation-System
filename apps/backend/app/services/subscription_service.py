...
class SubscriptionService:
    ...
    def get_user_plan(db: Session, user_id: int) -> dict:
        ...
        free_plan = db.execute(text("SELECT * FROM core.subscription_plans WHERE name = 'free'")).fetchone()
    ...
    def get_user_usage(db: Session, user_id: int) -> dict:
        ...
        result = db.execute(query, {"user_id": user_id, "year": now.year, "month": now.month}).fetchone()
        ...
        result = db.execute(insert_query, {"user_id": user_id, "year": now.year, "month": now.month}).fetchone()
    ...
    def increment_assessment_count(db: Session, user_id: int):
        ...
        db.execute(query, {"user_id": user_id, "year": now.year, "month": now.month})
        db.commit()
    ...
    def track_career_view(db: Session, user_id: int, career_id: int):
        ...
        db.execute(query, {"user_id": user_id, "career_id": f"[{career_id}]", "year": now.year, "month": now.month})
        db.commit()
