#!/usr/bin/env python3
"""
Script xóa sạch và nạp lại dữ liệu career_overview từ ONET API
...
class CareerOverviewReloader:
    def __init__(self):
        ...
        self.session.headers.update({
            'X-API-Key': ONET_V2_API_KEY,  # Sửa từ Authorization sang X-API-Key
    ...
    def get_onet_education_data(self, onet_code):
        """Lấy thông tin education từ ONET API"""
        try:
            url = f"{ONET_V2_BASE_URL}/database/rows/education_training_experience"
            
            # Thêm query parameter bắt buộc và filter cho ONET code
            params = {
                'client': 'github_trchitho_ai_b_1',
                'filter1': f'onetsoc_code.eq.{onet_code}'
            }
            ...
            if response.status_code == 200:
                ...
                self.consecutive_errors = 0
                data = response.json()
                return self.parse_education_data_from_database(data)
    ...
    def parse_education_data(self, data):
        """Parse education data từ ONET API response"""
        education_info = {
            'experience_text_en': '',
            'degree_text_en': ''
        }
            ...
            if 'response' in data and isinstance(data['response'], list):
                ...
                for item in data['response']:
                    if isinstance(item, dict):
                        ...
                        if 'education' in category.lower() or 'degree' in category.lower():
                            if description:
                                degree_parts.append(description)
                        elif 'experience' in category.lower() or 'training' in category.lower():
                            if description:
...
def main():
    print("=" * 70)
    print("🚀 RELOAD CAREER OVERVIEW FROM API")
    print("=" * 70)
