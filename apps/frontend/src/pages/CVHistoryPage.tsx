    ...
    >
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '0.75rem' }}>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.35rem' }}>
            <span style={{
              width: 36, height: 36, borderRadius: '50%', background: 'var(--neu-accent, #22c55e)',
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem' }}>
        {matchedSkills.length > 0 && (
          <div>
            <div style={{ fontSize: '0.68rem', fontWeight: 700, color: '#10b981', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 4 }}>✓ Đã có</div>
        ...
        {gapSkills.length > 0 && (
          <div>
            <div style={{ fontSize: '0.68rem', fontWeight: 700, color: '#ef4444', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 4 }}>✗ Cần học</div>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
              {gapSkills.map(s => (
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderTop: '1px solid var(--neu-border, #f3f4f6)', paddingTop: '0.75rem', marginTop: 'auto' }}>
        <div style={{ fontSize: '0.78rem', color: '#9ca3af' }}>
          {item.cv_filename && <span>📄 {item.cv_filename}</span>}
        </div>
        <span style={{ fontSize: '0.82rem', fontWeight: 700, color: 'var(--neu-accent, #22c55e)', display: 'flex', alignItems: 'center', gap: 4 }}>
          Xem chi tiết
        ...
        <div style={{ maxWidth: 1100, margin: '0 auto', padding: '1.75rem 1.5rem 0' }}>
          ...
          {!loading && analyses.length > 0 && (
            <div style={{
              ...
              <div style={{ flex: 1, minWidth: 200, position: 'relative' }}>
                <svg style={{ position: 'absolute', left: 10, top: '50%', transform: 'translateY(-50%)', pointerEvents: 'none' }} width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="#9ca3af" strokeWidth="2.5"><circle cx="11" cy="11" r="8"/><path d="M21 21l-4.35-4.35"/></svg>
                <input
                  value={search}
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
                ...
                {[{ k: 'date', l: 'Mới nhất' }, { k: 'match', l: 'Điểm cao nhất' }].map(o => (
                  <button key={o.k} onClick={() => setSortBy(o.k as any)}
                    style={{ padding: '0.35rem 0.85rem', borderRadius: 8, border: `1.5px solid ${sortBy === o.k ? 'var(--neu-accent,#22c55e)' : 'var(--neu-border,#e5e7eb)'}`, background: sortBy === o.k ? 'rgba(34,197,94,0.08)' : 'transparent', color: sortBy === o.k ? 'var(--neu-accent,#22c55e)' : '#6b7280', fontWeight: 600, fontSize: '0.8rem', cursor: 'pointer' }}>
                    {o.l}
                  </button>
                ))}
              </div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
                ...
                {[0, 40, 60, 80].map(v => (
                  <button key={v} onClick={() => setFilterMin(v)}
                    style={{ padding: '0.35rem 0.7rem', borderRadius: 8, border: `1.5px solid ${filterMin === v ? 'var(--neu-accent,#22c55e)' : 'var(--neu-border,#e5e7eb)'}`, background: filterMin === v ? 'rgba(34,197,94,0.08)' : 'transparent', color: filterMin === v ? 'var(--neu-accent,#22c55e)' : '#6b7280', fontWeight: 600, fontSize: '0.8rem', cursor: 'pointer' }}>
                    {v === 0 ? 'Tất cả' : `${v}%`}
                  </button>
                ))}
              </div>
          ...
          {loading && (
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '5rem 1rem', gap: '1rem', color: '#9ca3af' }}>
              <div style={{ width: 36, height: 36, border: '3px solid #e5e7eb', borderTopColor: 'var(--neu-accent,#22c55e)', borderRadius: '50%', animation: 'spin 0.8s linear infinite' }} />
          ...
          {error && (
            <div style={{ textAlign: 'center', padding: '4rem 1rem' }}>
              ...
              <button onClick={() => navigate('/skill-gap')} style={{ marginTop: '1rem', padding: '0.6rem 1.5rem', background: 'var(--neu-accent,#22c55e)', color: '#fff', border: 'none', borderRadius: 10, fontWeight: 700, cursor: 'pointer' }}>
                Phân tích CV ngay
              </button>
            </div>
          )}
          ...
          {!loading && !error && analyses.length === 0 && (
            <div style={{ textAlign: 'center', padding: '5rem 1rem' }}>
              ...
              <button onClick={() => navigate('/skill-gap')}
                style={{ padding: '0.75rem 2rem', background: 'linear-gradient(135deg, var(--neu-accent,#22c55e), #0d9488)', color: '#fff', border: 'none', borderRadius: 12, fontWeight: 700, fontSize: '0.95rem', cursor: 'pointer', boxShadow: '0 4px 14px rgba(34,197,94,0.3)' }}>
          ...
          {!loading && !error && analyses.length > 0 && filtered.length === 0 && (
            <div style={{ textAlign: 'center', padding: '3rem 1rem', color: '#9ca3af' }}>
              ...
              <button onClick={() => { setSearch(''); setFilterMin(0); }} style={{ marginTop: '0.75rem', fontSize: '0.85rem', color: 'var(--neu-accent,#22c55e)', background: 'none', border: 'none', cursor: 'pointer', fontWeight: 600 }}>
                Xóa bộ lọc
              </button>
            </div>
          )}
