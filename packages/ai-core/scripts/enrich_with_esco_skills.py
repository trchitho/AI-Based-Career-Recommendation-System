...
def analyze_esco_skills():
    ...
    high_value_skills = skills_df[
        skills_df['reuseLevel'].isin(['cross-sector', 'transversal'])
...
def enhance_jobs_with_esco_skills(esco_data, translation_map):
    ...
    for idx, job in enhanced_jobs.iterrows():
        ...
        job_description = str(job['description_vi']).lower()
        ...
        if any(word in job_title for word in ['quản lý', 'giám đốc', 'trưởng']):
            management_skills = [
                'lãnh đạo', 'quản lý nhóm', 'lập kế hoạch chiến lược', 
                'ra quyết định', 'quản lý dự án'
            ]
        ...
        if any(word in job_title for word in ['kỹ thuật', 'công nghệ', 'phần mềm', 'máy tính']):
            tech_skills = [
                'kỹ năng máy tính', 'phân tích dữ liệu', 'giải quyết vấn đề kỹ thuật',
                'tư duy logic', 'chú ý đến chi tiết'
            ]
        ...
        if any(word in job_title for word in ['y tế', 'bác sĩ', 'điều dưỡng', 'chăm sóc']):
            healthcare_skills = [
                'chăm sóc bệnh nhân', 'giao tiếp với bệnh nhân', 'làm việc dưới áp lực',
                'chú ý đến chi tiết', 'đồng cảm'
            ]
        ...
        if any(word in job_title for word in ['giáo viên', 'giảng viên', 'giáo dục']):
            education_skills = [
                'kỹ năng thuyết trình', 'giao tiếp', 'kiên nhẫn',
                'khả năng giải thích', 'quản lý lớp học'
            ]
        ...
        if any(word in job_title for word in ['bán hàng', 'tiếp thị', 'marketing']):
            sales_skills = [
                'kỹ năng bán hàng', 'giao tiếp khách hàng', 'thuyết phục',
                'đàm phán', 'xây dựng mối quan hệ'
            ]
        ...
        universal_skills = [
            ...
            'khả năng thích ứng', 'tư duy phản biện'
        ]
    ...
    # Show statistics
    original_avg_skills = current_jobs['tags_vi'].apply(
        lambda x: len(str(x).split('|')) if pd.notna(x) and str(x) else 0
    ).mean()
    
    enhanced_avg_skills = enhanced_jobs['tags_vi'].apply(
        lambda x: len(str(x).split('|')) if pd.notna(x) and str(x) else 0
    ).mean()
...
def update_etl_pipeline_for_enhanced_data():
    ...
    # Update the main ETL pipeline script to use enhanced data
    ...
    print(f"   python scripts/comprehensive_data_fix.py")
    print(f"   python scripts/ultimate_final_check.py")
