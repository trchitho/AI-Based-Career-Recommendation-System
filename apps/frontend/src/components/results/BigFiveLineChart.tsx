...
const BigFiveLineChart = ({ scores }: BigFiveLineChartProps) => {
  const data = [
    { name: 'O', fullName: 'Cởi Mở (Openness)', score: scores?.openness || 0, color: '#8B5CF6' },
    { name: 'C', fullName: 'Tận Tâm (Conscientiousness)', score: scores?.conscientiousness || 0, color: '#3B82F6' },
    { name: 'E', fullName: 'Hướng Ngoại (Extraversion)', score: scores?.extraversion || 0, color: '#10B981' },
    { name: 'A', fullName: 'Dễ Chịu (Agreeableness)', score: scores?.agreeableness || 0, color: '#F59E0B' },
    { name: 'N', fullName: 'Nhạy Cảm (Neuroticism)', score: scores?.neuroticism || 0, color: '#EF4444' },
  ];
