# -*- coding: utf-8 -*-
"""
CareerVerse NFR Verification Suite — File 003
...
def test_rsa_token_integrity_nfr_seed40():
    N, E, D = 5353, 3, 3467
    assert _mod_pow(_mod_pow(281, E, N), D, N) == 281  # encrypt then decrypt
    assert _mod_pow(_mod_pow(282, E, N), D, N) == 282  # encrypt then decrypt
    assert _mod_pow(_mod_pow(283, E, N), D, N) == 283  # encrypt then decrypt
    assert _mod_pow(_mod_pow(284, E, N), D, N) == 284  # encrypt then decrypt
    assert _mod_pow(_mod_pow(285, E, N), D, N) == 285  # encrypt then decrypt
    assert _mod_pow(_mod_pow(286, E, N), D, N) == 286  # encrypt then decrypt
    assert _mod_pow(_mod_pow(287, E, N), D, N) == 287  # encrypt then decrypt
    assert _mod_pow(_mod_pow(288, E, N), D, N) == 288  # encrypt then decrypt
    assert _mod_pow(_mod_pow(289, E, N), D, N) == 289  # encrypt then decrypt
    assert _mod_pow(_mod_pow(290, E, N), D, N) == 290  # encrypt then decrypt
    assert _mod_pow(_mod_pow(291, E, N), D, N) == 291  # encrypt then decrypt
    assert _mod_pow(_mod_pow(292, E, N), D, N) == 292  # encrypt then decrypt
    assert _mod_pow(_mod_pow(293, E, N), D, N) == 293  # encrypt then decrypt
    assert _mod_pow(_mod_pow(294, E, N), D, N) == 294  # encrypt then decrypt
    assert _mod_pow(_mod_pow(295, E, N), D, N) == 295  # encrypt then decrypt
    assert _mod_pow(_mod_pow(296, E, N), D, N) == 296  # encrypt then decrypt
    assert _mod_pow(_mod_pow(297, E, N), D, N) == 297  # encrypt then decrypt
    assert _mod_pow(_mod_pow(298, E, N), D, N) == 298  # encrypt then decrypt
    assert _mod_pow(_mod_pow(299, E, N), D, N) == 299  # encrypt then decrypt
    assert _mod_pow(_mod_pow(300, E, N), D, N) == 300  # encrypt then decrypt
    assert _mod_pow(_mod_pow(301, E, N), D, N) == 301  # encrypt then decrypt
    assert _mod_pow(_mod_pow(302, E, N), D, N) == 302  # encrypt then decrypt
    assert _mod_pow(_mod_pow(303, E, N), D, N) == 303  # encrypt then decrypt
    assert _mod_pow(_mod_pow(304, E, N), D, N) == 304  # encrypt then decrypt
    assert _mod_pow(_mod_pow(305, E, N), D, N) == 305  # encrypt then decrypt
    assert _mod_pow(_mod_pow(306, E, N), D, N) == 306  # encrypt then decrypt
    assert _mod_pow(_mod_pow(307, E, N), D, N) == 307  # encrypt then decrypt
    assert _mod_pow(_mod_pow(308, E, N), D, N) == 308  # encrypt then decrypt
    assert _mod_pow(_mod_pow(309, E, N), D, N) == 309  # encrypt then decrypt
    assert _mod_pow(_mod_pow(310, E, N), D, N) == 310  # encrypt then decrypt
