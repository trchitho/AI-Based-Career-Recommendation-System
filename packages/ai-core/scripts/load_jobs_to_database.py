...
from ai_core.db import get_session, engine
...
class JobsDatabaseLoader:
    ...
    def load_jobs_data(self):
        ...
        try:
            ...
            # Show sample
            print(f"\n📋 Sample jobs:")
            for i, row in self.jobs_df.head(3).iterrows():
                ...
                for tag in new_tags:
                    try:
                        result = self.session.execute(
                            text("INSERT INTO core.career_tags (name) VALUES (:name) RETURNING id"),
                            {'name': tag}
                        )
    ...
    def run(self):
        ...
        try:
            ...
            if self.stats['errors'] == 0:
                ...
                print(f"   All jobs data has been loaded into PostgreSQL")
            else:
