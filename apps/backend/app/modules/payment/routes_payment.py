    ...
    try:
        ...
        try:
            ...
            new_session.execute(
                ...
                VALUES 
                (:actor_id, :action, :entity, :entity_id, CAST(:data_json AS jsonb), 
     :user_id, :resource_type, :resource_id, CAST(:details AS jsonb), 
     :ip_address, NOW())
            """
                ),
                {
...
def create_payment(
    ...
    try:
        ...
        db.rollback()
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Lỗi tạo thanh toán: {str(e)}")
    ...
    try:
        ...
        try:
            ...
            upgrade_success = SubscriptionService.upgrade_user_subscription(
                user_id=payment.user_id, plan_name=plan_name.strip(), payment_id=payment.id, session=db
            )
...
def payment_redirect(
    ...
    # Tìm payment record
    payment = db.query(Payment).filter((Payment.app_trans_id == app_trans_id) | (Payment.order_id == order_id)).first()
...
def get_payment_history(
    ...
    try:
        ...
        if current_sub.get("plan_name") == "Free":
            ...
            success_payment = (
                db.query(Payment)
                .filter(Payment.user_id == user_id, func.upper(Payment.status) == "SUCCESS")
            ...
            if success_payment:
                ...
                # Auto-upgrade
                upgrade_success = SubscriptionService.upgrade_user_subscription(
                    user_id=user_id, plan_name=plan_name, payment_id=success_payment.id, session=db
                )
    ...
    try:
        ...
        pending_payments = db.query(Payment).filter(Payment.user_id == user_id, func.upper(Payment.status) == "PENDING").all()
    ...
    payments = (
        db.query(Payment).filter(Payment.user_id == user_id).order_by(Payment.created_at.desc()).offset(skip).limit(limit).all()
    )
...
def admin_search_users(
    ...
    try:
        ...
        return [{"id": row.id, "email": row.email, "full_name": row.full_name} for row in rows]
