...
const PHASE_COLORS = ['#6366f1', '#0891b2', '#059669', '#d97706'];
    ...
    <div style={{ background: 'white', borderRadius: 16, padding: '1.5rem', boxShadow: '0 2px 12px rgba(0,0,0,0.08)' }}>
        ...
        {plan.milestones?.map((m, i) => (
          <React.Fragment key={i}>
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', minWidth: 100 }}>
              <div style={{
                width: 36, height: 36, borderRadius: '50%', background: '#6366f1', color: 'white',
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
        {plan.phases?.map((phase, idx) => {
          ...
          return (
            <div key={idx} style={{ border: `1.5px solid ${isOpen ? color : '#e2e8f0'}`, borderRadius: 12, overflow: 'hidden', transition: 'border-color 0.2s' }}>
              {/* Phase header */}
              <button
                onClick={() => setOpenPhase(isOpen ? -1 : idx)}
                style={{
                  width: '100%', display: 'flex', alignItems: 'center', gap: '0.75rem',
                  padding: '0.85rem 1.1rem', background: isOpen ? `${color}10` : 'white',
              ...
              >
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ fontWeight: 700, color: '#1e293b', fontSize: '0.95rem' }}>{phase.title}</div>
                  <div style={{ fontSize: '0.78rem', color: '#64748b' }}>{phase.weeks} · {phase.focus}</div>
                </div>
              ...
              {/* Phase detail */}
              {isOpen && (
                <div style={{ padding: '1rem 1.1rem', borderTop: `1px solid ${color}30`, background: '#fafafa' }}>
                  <div style={{ marginBottom: '0.75rem' }}>
                    <div style={{ fontSize: '0.8rem', fontWeight: 600, color: '#475569', marginBottom: '0.4rem' }}>KỸ NĂNG TRONG GIAI ĐOẠN</div>
      ...
      {/* Total time */}
      <div style={{ marginTop: '1.25rem', padding: '0.75rem 1rem', background: '#f0f9ff', border: '1px solid #bae6fd', borderRadius: 10, display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
        <span style={{ fontSize: '1.2rem' }}>⏱️</span>
        <span style={{ fontSize: '0.88rem', color: '#0369a1', fontWeight: 600 }}>
