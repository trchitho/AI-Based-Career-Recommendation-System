...
RETR_DIR     = Path(os.getenv("RETR_MODEL_DIR", "models/vi_sbert_768"))

def _read_model_name(p: Path) -> str:
    if not p.exists():
        return ""
    raw = p.read_text(encoding="utf-8", errors="ignore")
    return raw.replace("\ufeff", "").strip()  # strip BOM nếu có

SBERT_NAME = os.getenv("RETR_MODEL_NAME", _read_model_name(RETR_DIR / "tokenizer_name.txt")) or \
...
def search(req: SearchReq):
    ...
    # Trả JSON UTF-8 (ensure_ascii=False)
    return JSONResponse({"topk": req.topk, "results": hits}, media_type="application/json; charset=utf-8")

# ===================== PhoBERT essay → ai.user_embeddings =====================
PHO_DIR = Path(os.getenv("PHOBERT_DIR", "models/riasec_phobert"))
if (PHO_DIR / "tokenizer_name.txt").exists():
    PHO_BACKBONE_NAME = _read_model_name(PHO_DIR / "tokenizer_name.txt") or "vinai/phobert-base"
else:
...
# (Optional heads) – không bắt buộc cho demo
RIASEC_HEAD = None
BIG5_HEAD   = None

def _predict_traits(_text: str):
    return None, None
...
class InferReq(BaseModel):
    ...
    essay_text: str
    lang: str = "vi"
    test_scores: Optional[ScoresIn] = None
