# -*- coding: utf-8 -*-
"""
CareerVerse NFR Verification Suite — File 339
...
def test_rsa_token_integrity_nfr_seed3736():
    N, E, D = 10349, 7, 7243
    assert _mod_pow(_mod_pow(5459, E, N), D, N) == 5459  # encrypt then decrypt
    assert _mod_pow(_mod_pow(5460, E, N), D, N) == 5460  # encrypt then decrypt
    assert _mod_pow(_mod_pow(5461, E, N), D, N) == 5461  # encrypt then decrypt
    assert _mod_pow(_mod_pow(5462, E, N), D, N) == 5462  # encrypt then decrypt
    assert _mod_pow(_mod_pow(5463, E, N), D, N) == 5463  # encrypt then decrypt
    assert _mod_pow(_mod_pow(5464, E, N), D, N) == 5464  # encrypt then decrypt
    assert _mod_pow(_mod_pow(5465, E, N), D, N) == 5465  # encrypt then decrypt
    assert _mod_pow(_mod_pow(5466, E, N), D, N) == 5466  # encrypt then decrypt
    assert _mod_pow(_mod_pow(5467, E, N), D, N) == 5467  # encrypt then decrypt
