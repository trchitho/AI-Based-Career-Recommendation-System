...
def main():
    ...
    # Final summary
    print(f"\n{'='*80}")
    print(f"📋 ETL PIPELINE EXECUTION SUMMARY")
    print(f"{'='*80}")
    ...
    if successful_steps == total_steps:
        ...
        print(f"   All data has been processed and loaded into the database.")
        print(f"   The AI Career Recommendation System is ready for use.")
    else:
        print(f"\n⚠️  ETL PIPELINE COMPLETED WITH ISSUES")
        print(f"   {successful_steps}/{total_steps} steps completed successfully.")
        print(f"   Please review the failed steps and run them individually if needed.")
    
    print(f"\n📊 Database Status:")
    print(f"   - Core tables: careers, career_interests, career_tags, mappings")
    print(f"   - AI tables: retrieval_jobs_visbert (for vector search)")
    print(f"   - Data quality: Vietnamese encoding, RIASEC scores, tag coverage")
    
    print(f"\n🔧 Next Steps:")
    print(f"   1. Review the status report above")
    print(f"   2. Fix any remaining encoding issues if needed")
    print(f"   3. Generate actual embeddings (currently using dummy vectors)")
    print(f"   4. Test the recommendation system with sample queries")
