        ...
        <div className="border rounded p-4 h-[60vh] overflow-y-auto bg-white dark:bg-gray-900">
          {messages.map((m, i) => (
            <div key={i} className={`mb-3 ${m.role==='user' ? 'text-right' : 'text-left'}`}>
              <div className={`inline-block px-3 py-2 rounded ${m.role==='user' ? 'bg-purple-600 text-white' : m.role==='assistant' ? 'bg-gray-200 dark:bg-gray-800 text-gray-900 dark:text-gray-100' : 'bg-yellow-100 text-gray-800'}`}>{m.content}</div>
            </div>
          ))}
          <div ref={endRef} />
        </div>
        <form onSubmit={send} className="mt-4 flex gap-2">
          <input className="flex-1 border rounded px-3 py-2" placeholder="Ask about careers, skills, learning..." value={input} onChange={(e)=>setInput(e.target.value)} />
