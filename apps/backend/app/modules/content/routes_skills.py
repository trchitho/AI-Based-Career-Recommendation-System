...
def get_skills(
    ...
    try:
        ...
        subquery = (
            ...
            .group_by(CareerKSA.onet_code, CareerKSA.ksa_type, CareerKSA.name)
            .subquery()
        )
        ...
        return SkillsListResponse(items=skill_items, total=total, page=page, per_page=per_page, total_pages=total_pages)
...
def get_skills_stats(
    ...
    try:
        ...
        return {
            ...
            "ksa_type_distribution": [{"type": row[0], "count": row[1]} for row in ksa_stats],
            "top_onet_codes": [{"onet_code": row[0], "count": row[1]} for row in onet_stats],
        }

    except Exception as e:
        print(f"[skills] get_skills_stats error: {repr(e)}")
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to get skills stats")
