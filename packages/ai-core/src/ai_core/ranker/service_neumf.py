...
def _load_json(path: Path) -> dict:
    if not path.exists():
        raise FileNotFoundError(f"{path} not found. Hãy chạy build_feats_from_db.py để sinh file này.")
...
def _lazy_load():
    ...
    if _USER_FEATS is None or _ITEM_FEATS is None:
        ...
        print(f"[BOOT][B4] Loaded feats: users={len(_USER_FEATS)}, items={len(_ITEM_FEATS)}")

    if _MODEL is None:
        if not _MODEL_PATH.exists():
            raise FileNotFoundError(f"Model checkpoint not found: {_MODEL_PATH}. Hãy chạy train.py để sinh best.pt.")
...
def infer_scores(user_id: int, candidates: list[Candidate]) -> list[ScoredItem]:
    ...
    if uid not in user_feats:
        raise ValueError(f"user_id={user_id} không có trong user_feats (users={len(user_feats)}). Hãy kiểm tra build_feats_from_db.")

    # Giữ lại chỉ những job có features
    valid_cands: list[Candidate] = [c for c in candidates if c.job_id in item_feats]
    if not valid_cands:
        return []
...
def _demo_for_user(user_id: int, top_n: int):
    ...
    for s in scored[:top_n]:
        print(f"  {s.job_id}: rank={s.rank_score:.4f} (sim={s.sim_score:.4f}, cf={s.cf_score:.4f})")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--all-users", action="store_true", help="Chạy cho tất cả user có embedding")
