...
def _fetch_sections(conn: psycopg.Connection, code: str, language: str = "en") -> Dict[str, Any]:
    ...
    with conn.cursor(row_factory=dict_row) as cur:
        ...
        # 7. Detailed Work Activities (optional table)
        dwas = []
        try:
            cur.execute(
                f"""
        ...
        try:
            ...
            logger.warning(f"[BFF] work_activity_summary query failed: {_e}")
            try: conn.rollback()
            except Exception: pass

        # 12. Work Context (optional table)
        work_context = []
        try:
            cur.execute(
                f"""
                SELECT element_id,
                       {get_lang_col("element_name", "element_name_vn")} AS element_name,
                       scale_id, category,
                       {get_lang_col("category_description", "category_description_vn")} AS category_description,
                       data_value, n, standard_error
                FROM core.career_work_context
                WHERE onet_code = %s ORDER BY scale_id, data_value DESC
                """, (code,),
            )
            work_context = cur.fetchall()
        except Exception as _e:
            logger.warning(f"[BFF] work_context query failed: {_e}")
            try: conn.rollback()
            except Exception: pass
