...
def chat_with_bot(request: Request, chat_message: ChatMessage):
    ...
    try:
        ...
        # Tạo phản hồi từ AI
        try:
            response = gemini_service.generate_response(chat_message.message, chat_message.context)
...
def get_career_advice(request: Request, advice_request: CareerAdviceRequest):
    ...
    try:
        ...
        return ChatResponse(response=advice, timestamp=datetime.now().isoformat(), success=True)
    except Exception as e:
...
def get_skill_development_plan(request: Request, skill_request: SkillDevelopmentRequest):
    ...
    try:
        ...
        gemini_service = GeminiChatbotService()
        plan = gemini_service.get_skill_development_plan(skill_request.current_skills, skill_request.target_job)

        return ChatResponse(response=plan, timestamp=datetime.now().isoformat(), success=True)
    except Exception as e:
...
def analyze_job_market(request: Request, market_request: JobMarketAnalysisRequest):
    ...
    try:
        ...
        gemini_service = GeminiChatbotService()
        analysis = gemini_service.analyze_job_market(market_request.job_title, market_request.location)

        return ChatResponse(response=analysis, timestamp=datetime.now().isoformat(), success=True)
    except Exception as e:
...
def chatbot_health_check():
    ...
    try:
        ...
        return {"status": "healthy", "gemini_api": "connected", "timestamp": datetime.now().isoformat()}
    except Exception as e:
