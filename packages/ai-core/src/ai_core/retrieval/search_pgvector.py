...
def expand_query_vi(q: str) -> str:
    """Thêm ít từ khóa kỹ năng phổ biến để tăng recall nhưng không làm lệch ý gốc."""
    q_low = q.lower()
    extra = []
    if "trực quan hóa" in q_low or "truc quan hoa" in q_low:
...
def main():
    ...
    ap.add_argument("--user_riasec", nargs="*", type=float, default=None, help="6 số [R,I,A,S,E,C]")
    ap.add_argument(
        "--min_score", type=float, default=None, help="Ngưỡng final_score; bỏ qua nếu không đặt"
    )
