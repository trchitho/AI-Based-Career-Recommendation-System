...
def get_payment_stats(
    ...
    try:
        ...
        completed_payments = db.query(Payment).filter(Payment.status == PaymentStatus.SUCCESS).count()
        completed_amount = db.query(func.sum(Payment.amount)).filter(Payment.status == PaymentStatus.SUCCESS).scalar() or 0
        ...
        # Thanh toán bị hủy
        cancelled_payments = db.query(Payment).filter(Payment.status == PaymentStatus.CANCELLED).count()

        # Thanh toán hôm nay
        today = date.today()
        today_payments = db.query(Payment).filter(func.date(Payment.created_at) == today).count()
        today_amount = db.query(func.sum(Payment.amount)).filter(func.date(Payment.created_at) == today).scalar() or 0
...
def get_payments(
    ...
    try:
        ...
        return PaymentListResponse(items=payment_items, total=total, page=page, per_page=per_page, total_pages=total_pages)

    except Exception as e:
        print(f"[admin-payments] get_payments error: {repr(e)}")
        # Trả về dữ liệu mock nếu có lỗi database
        return PaymentListResponse(items=[], total=0, page=page, per_page=per_page, total_pages=0)
...
def get_user_payment_history(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to get user payment history: {str(e)}"
        )
...
def search_users(
    ...
    try:
        search_term = f"%{q}%"
        users = db.query(User).filter(or_(User.email.ilike(search_term), User.full_name.ilike(search_term))).limit(limit).all()
...
def export_payments(
    ...
    try:
        ...
        if format == "json":
            ...
            return JSONResponse(
                content={"data": export_data, "total": len(export_data), "exported_at": datetime.now().isoformat()}
            )
        else:  # CSV
            import csv
            import io
            ...
            return StreamingResponse(
                ...
                media_type="text/csv",
                headers={
                    "Content-Disposition": f"attachment; filename=payments_export_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
                },
            )

    except Exception as e:
        print(f"[admin-payments] export_payments error: {repr(e)}")
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to export payments: {str(e)}")
...
def get_mock_payments(
    ...
    return PaymentListResponse(items=items, total=total, page=page, per_page=per_page, total_pages=total_pages)
