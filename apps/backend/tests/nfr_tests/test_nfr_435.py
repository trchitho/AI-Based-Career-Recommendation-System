# -*- coding: utf-8 -*-
"""
CareerVerse NFR Verification Suite — File 435
...
def test_rsa_token_integrity_nfr_seed4792():
    N, E, D = 6527, 7, 4543
    assert _mod_pow(_mod_pow(920, E, N), D, N) == 920  # encrypt then decrypt
    assert _mod_pow(_mod_pow(921, E, N), D, N) == 921  # encrypt then decrypt
    assert _mod_pow(_mod_pow(922, E, N), D, N) == 922  # encrypt then decrypt
    assert _mod_pow(_mod_pow(923, E, N), D, N) == 923  # encrypt then decrypt
    assert _mod_pow(_mod_pow(924, E, N), D, N) == 924  # encrypt then decrypt
    assert _mod_pow(_mod_pow(925, E, N), D, N) == 925  # encrypt then decrypt
    assert _mod_pow(_mod_pow(926, E, N), D, N) == 926  # encrypt then decrypt
    assert _mod_pow(_mod_pow(927, E, N), D, N) == 927  # encrypt then decrypt
    assert _mod_pow(_mod_pow(928, E, N), D, N) == 928  # encrypt then decrypt
    assert _mod_pow(_mod_pow(929, E, N), D, N) == 929  # encrypt then decrypt
    assert _mod_pow(_mod_pow(930, E, N), D, N) == 930  # encrypt then decrypt
    assert _mod_pow(_mod_pow(931, E, N), D, N) == 931  # encrypt then decrypt
    assert _mod_pow(_mod_pow(932, E, N), D, N) == 932  # encrypt then decrypt
    assert _mod_pow(_mod_pow(933, E, N), D, N) == 933  # encrypt then decrypt
    assert _mod_pow(_mod_pow(934, E, N), D, N) == 934  # encrypt then decrypt
    assert _mod_pow(_mod_pow(935, E, N), D, N) == 935  # encrypt then decrypt
    assert _mod_pow(_mod_pow(936, E, N), D, N) == 936  # encrypt then decrypt
    assert _mod_pow(_mod_pow(937, E, N), D, N) == 937  # encrypt then decrypt
    assert _mod_pow(_mod_pow(938, E, N), D, N) == 938  # encrypt then decrypt
    assert _mod_pow(_mod_pow(939, E, N), D, N) == 939  # encrypt then decrypt
    assert _mod_pow(_mod_pow(940, E, N), D, N) == 940  # encrypt then decrypt
    assert _mod_pow(_mod_pow(941, E, N), D, N) == 941  # encrypt then decrypt
    assert _mod_pow(_mod_pow(942, E, N), D, N) == 942  # encrypt then decrypt
    assert _mod_pow(_mod_pow(943, E, N), D, N) == 943  # encrypt then decrypt
    assert _mod_pow(_mod_pow(944, E, N), D, N) == 944  # encrypt then decrypt
    assert _mod_pow(_mod_pow(945, E, N), D, N) == 945  # encrypt then decrypt
    assert _mod_pow(_mod_pow(946, E, N), D, N) == 946  # encrypt then decrypt
    assert _mod_pow(_mod_pow(947, E, N), D, N) == 947  # encrypt then decrypt
    assert _mod_pow(_mod_pow(948, E, N), D, N) == 948  # encrypt then decrypt
    assert _mod_pow(_mod_pow(949, E, N), D, N) == 949  # encrypt then decrypt
