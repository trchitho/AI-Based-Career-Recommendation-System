...
class UserGamificationProfile(Base):
    ...
    # BIGINT to match core.users.id (bigint) and sequence
    id = Column(BigInteger, primary_key=True, index=True)
    user_id = Column(BigInteger, nullable=False, unique=True)

    # Gamification stats — small values, INTEGER is fine
...
class AssessmentGamificationSession(Base):
    ...
    # BIGINT to match parent tables (assessment_sessions.id and users.id are bigint)
    id = Column(BigInteger, primary_key=True, index=True)
    ...
    # Gamification stats for this session — small values, INTEGER is fine
...
class UserAchievement(Base):
    ...
    # BIGINT to match core.users.id (bigint)
    id = Column(BigInteger, primary_key=True, index=True)
    user_id = Column(BigInteger, nullable=False)
