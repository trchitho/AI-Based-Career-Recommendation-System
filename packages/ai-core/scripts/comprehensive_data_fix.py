...
def fix_industry_categories():
    ...
    try:
        ...
        updated_count = 0
        for onet_code, title_vi in null_careers:
...
def fix_all_other_riasec_scores(group_averages):
    ...
    try:
        ...
        updated_count = 0
        for onet_code, title_vi in all_other_jobs:
            major_group = onet_code[:2]
            ...
            if major_group in group_averages:
                ...
                """), {
                    'r': r_score, 'i': i_score, 'a': a_score,
                    's': s_score, 'e': e_score, 'c': c_score,
                    'onet_code': onet_code
                })
    ...
    try:
        ...
        if three_letter_codes:
            ...
            for code in three_letter_codes:
                try:
                    session.execute(
                        text("INSERT INTO core.riasec_labels (code) VALUES (:code)"),
                        {'code': code}
                    )
                    inserted_count += 1
    ...
    try:
        ...
        for career_id, r, i, a, s, e, c in career_riasec_data:
            ...
            if len(top_codes) >= 2:
                ...
                if combo_2 in riasec_labels:
                    mapping_data.append({
                        'career_id': career_id,
                        'label_id': riasec_labels[combo_2]
                    })
            ...
            if len(top_codes) >= 3:
                ...
                if combo_3 in riasec_labels:
                    mapping_data.append({
                        'career_id': career_id,
                        'label_id': riasec_labels[combo_3]
                    })
        ...
        if mapping_data:
            ...
            for i in range(0, len(mapping_data), batch_size):
                ...
                session.execute(
                    text("INSERT INTO core.career_riasec_map (career_id, label_id) VALUES (:career_id, :label_id)"),
                    batch
                )
                session.commit()
                inserted += len(batch)
