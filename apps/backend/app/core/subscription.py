...
class SubscriptionService:
    ...
    def get_user_usage(user_id: int, feature_type: str, session: Session) -> Dict[str, Any]:
        ...
        result = session.execute(query, {"user_id": user_id, "feature_type": feature_type}).fetchone()

        if result:
            return {"usage_count": result.usage_count, "last_reset_date": result.last_reset_date}
        else:
            ...
            session.execute(insert_query, {"user_id": user_id, "feature_type": feature_type})
            session.commit()

            return {"usage_count": 0, "last_reset_date": date.today()}
    ...
    def increment_usage(user_id: int, feature_type: str, session: Session) -> bool:
        ...
        try:
            session.execute(query, {"user_id": user_id, "feature_type": feature_type})
            session.commit()
            return True
    ...
    @staticmethod
    def check_feature_access(user_id: int, feature_type: str, session: Session, level: Optional[int] = None) -> Dict[str, Any]:
        """
        ...
        if feature_type == "career_view":
            ...
            if limit == -1:  # Unlimited (Premium/Pro only)
                return {"allowed": True, "reason": "Premium access", "current_usage": current_usage, "limit": -1}
            else:
            ...
            if max_level == -1:  # Unlimited (Premium/Pro only)
                return {"allowed": True, "reason": "Premium access", "current_usage": level or 1, "limit": -1}
            else:
            ...
            return {"allowed": False, "reason": "Unknown feature type", "current_usage": 0, "limit": 0}
    ...
    def upgrade_user_subscription(user_id: int, plan_name: str, payment_id: int, session: Session) -> bool:
        ...
        try:
            ...
            session.execute(
                ...
                {"user_id": user_id, "plan_id": plan_result.id, "payment_id": payment_id, "expires_at": expires_at},
            )
